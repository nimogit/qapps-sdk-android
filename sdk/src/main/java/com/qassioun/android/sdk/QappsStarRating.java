...
public class QappsStarRating {
     ...
     * @param callback
     */
    public static void showStarRating(final Context context, final QappsStore cs, final QappsStarRating.RatingCallback callback){
    ...
    public static void showStarRatingCustom(
            ...
            final QappsStarRating.RatingCallback callback) {

        if(!(context instanceof Activity)) {
            if (Qapps.sharedInstance().isLoggingEnabled()) {
                Log.e(Qapps.TAG, "Can't show star rating dialog, the provided context is not based off a activity");
            }
            return;
        }
        ...
        ratingBar.setOnRatingBarChangeListener(new RatingBar.OnRatingBarChangeListener() {
            ...
            public void onRatingChanged(RatingBar ratingBar, float v, boolean b) {
                ...
                if(Qapps.sharedInstance().getConsent(Qapps.QappsFeatureNames.starRating)) {
                    Map<String, String> segm = new HashMap<>();
                    segm.put("platform", "android");
                    segm.put("app_version", DeviceInfo.getAppVersion(context));
                    segm.put("rating", "" + rating);

                    Qapps.sharedInstance().recordEvent(STAR_RATING_EVENT_KEY, segm, 1);
                }
    ...
    static class StarRatingPreferences {
        ...
        JSONObject toJSON() {
            ...
            try {
                ...
                if (Qapps.sharedInstance().isLoggingEnabled()) {
                    Log.w(Qapps.TAG, "Got exception converting an StarRatingPreferences to JSON", e);
                }
            }
        ...
        static StarRatingPreferences fromJSON(final JSONObject json) {
            ...
            if(json != null) {
                try {
                    ...
                    if (Qapps.sharedInstance().isLoggingEnabled()) {
                        Log.w(Qapps.TAG, "Got exception converting JSON to a StarRatingPreferences", e);
                    }
                }
            }
     ...
     */
    public static void setStarRatingInitConfig(final QappsStore cs, final int limit, final String starRatingTextTitle, final String starRatingTextMessage, final String starRatingTextDismiss) {
        StarRatingPreferences srp = loadStarRatingPreferences(cs);
     ...
     * @return
     */
    private static StarRatingPreferences loadStarRatingPreferences(final QappsStore cs) {
     ...
     * @param srp
     */
    private static void saveStarRatingPreferences(final QappsStore cs, final StarRatingPreferences srp) {
     ...
     * @param shouldShow
     */
    public static void setShowDialogAutomatically(final QappsStore cs, final boolean shouldShow) {
        StarRatingPreferences srp = loadStarRatingPreferences(cs);
     ...
     */
    public static void setStarRatingDisableAskingForEachAppVersion(final QappsStore cs, final boolean disableAsking) {
        StarRatingPreferences srp = loadStarRatingPreferences(cs);
     ...
     */
    public static void registerAppSession(final Context context, final QappsStore cs, final RatingCallback starRatingCallback) {
        StarRatingPreferences srp = loadStarRatingPreferences(cs);
     ...
     */
    public static int getAutomaticStarRatingSessionLimit(final QappsStore cs){
        StarRatingPreferences srp = loadStarRatingPreferences(cs);
     ...
     * @return
     */
    public static int getCurrentVersionsSessionCount(final QappsStore cs){
        StarRatingPreferences srp = loadStarRatingPreferences(cs);
     ...
     */
    public static void clearAutomaticStarRatingSessionCount(final QappsStore cs){
        StarRatingPreferences srp = loadStarRatingPreferences(cs);
        srp.sessionAmount = 0;
        saveStarRatingPreferences(cs, srp);
    }
     ...
     * @param isCancellable
     */
    public static void setIfRatingDialogIsCancellable(final QappsStore cs, final boolean isCancellable){
        StarRatingPreferences srp = loadStarRatingPreferences(cs);
        srp.isDialogCancellable = isCancellable;
        saveStarRatingPreferences(cs, srp);
    }

    /// Qapps webDialog user rating
    ...
    protected static synchronized void showFeedbackPopup(final String widgetId, final String closeButtonText, final Activity activity, final Qapps qapps, final ConnectionQueue connectionQueue_, final FeedbackRatingCallback devCallback){
        if (Qapps.sharedInstance().isLoggingEnabled()) {
            Log.d(Qapps.TAG, "Showing Feedback popup for widget id: [" + widgetId + "]");
        }

        if(widgetId == null || widgetId.isEmpty()){
            if(devCallback != null){
                devCallback.callback("Qapps widgetId cannot be null or empty");
            }
            throw new IllegalArgumentException("Qapps widgetId cannot be null or empty");
        }

        if(qapps.getConsent(Qapps.QappsFeatureNames.starRating)) {
            //check the device type
            final boolean deviceIsPhone;
            final boolean deviceIsTablet;
            final boolean deviceIsTv;

            deviceIsTv = QappsStarRating.isDeviceTv(activity);

            if(!deviceIsTv) {
                deviceIsPhone = !QappsStarRating.isDeviceTablet(activity);
                deviceIsTablet = QappsStarRating.isDeviceTablet(activity);
            } else {
            ...
            try {
                ...
                if (Qapps.sharedInstance().isLoggingEnabled()) {
                    Log.e(Qapps.TAG, "IOException while checking for rating widget availability :[" + e.toString() + "]");
                }
            ...
            if (Qapps.sharedInstance().isLoggingEnabled()) {
                Log.d(Qapps.TAG, "rating widget url :[" + ratingWidgetUrl + "]");
            }
            ...
            (new ImmediateRequestMaker()).execute(urlConnection, false, new InternalFeedbackRatingCallback() {
                ...
                public void callback(JSONObject checkResponse) {
                    if(checkResponse == null){
                        if (Qapps.sharedInstance().isLoggingEnabled()) {
                            Log.d(Qapps.TAG, "Not possible to show Feedback popup for widget id: [" + widgetId + "], probably a lack of connection to the server");
                        }
                        if(devCallback != null){
    ...
    protected static class ImmediateRequestMaker extends AsyncTask<Object, Void, JSONObject> {
        ...
        protected JSONObject doInBackground(Object... params) {
            if (Qapps.sharedInstance().isLoggingEnabled()) {
                Log.v(Qapps.TAG, "Starting ImmediateRequestMaker request");
            }
            ...
            try {
                if(requestShouldBeDelayed){
                    ...
                    if (Qapps.sharedInstance().isLoggingEnabled()) {
                        Log.v(Qapps.TAG, "ImmediateRequestMaker request should be delayed, waiting for 10.5 seconds");
                    }
                    ...
                    try {
                        ...
                        if (Qapps.sharedInstance().isLoggingEnabled()) {
                            Log.w(Qapps.TAG, "While waiting for 10 seconds in ImmediateRequestMaker, sleep was interrupted");
                        }
                    }
                }
                ...
                if(wasSuccess) {
                    ...
                    if (Qapps.sharedInstance().isLoggingEnabled()) {
                        Log.e(Qapps.TAG, "Encountered problem while making a immediate server request, :[" + buffer.toString() + "]");
                    }
                    return null;
                }
