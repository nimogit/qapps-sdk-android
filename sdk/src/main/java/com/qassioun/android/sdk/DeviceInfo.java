...
class DeviceInfo {
    ...
    static String getResolution(final Context context) {
        ...
        try {
            ...
            if (Qapps.sharedInstance().isLoggingEnabled()) {
                Log.i(Qapps.TAG, "Device resolution cannot be determined");
            }
        }
        return resolution;
    }
    ...
    static String getCarrier(final Context context) {
        ...
        if (carrier == null || carrier.length() == 0) {
            carrier = "";
            if (Qapps.sharedInstance().isLoggingEnabled()) {
                Log.i(Qapps.TAG, "No carrier found");
            }
        }
        return carrier;
    }
    ...
    static String getAppVersion(final Context context) {
        String result = Qapps.DEFAULT_APP_VERSION;
        try {
            result = context.getPackageManager().getPackageInfo(context.getPackageName(), 0).versionName;
        }
        catch (PackageManager.NameNotFoundException e) {
            if (Qapps.sharedInstance().isLoggingEnabled()) {
                Log.i(Qapps.TAG, "No app version found");
            }
        }
        return result;
    }
    ...
    static String getStore(final Context context) {
        ...
        try {
            ...
            if (Qapps.sharedInstance().isLoggingEnabled()) {
                Log.i(Qapps.TAG, "Can't get Installer package [getStore]");
            }
        }
        if (result == null || result.length() == 0) {
            result = "";
            if (Qapps.sharedInstance().isLoggingEnabled()) {
                Log.i(Qapps.TAG, "No store found [getStore]");
            }
        }
        return result;
    }
     ...
     * https://qassioun.com/resources/reference/server-api
     */
