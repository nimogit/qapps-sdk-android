...
class CrashDetails {
    ...
    static void addLog(String record) {
        ...
        if(recordLength > maxBreadcrumbSize){
            if(Qapps.sharedInstance().isLoggingEnabled()){
                Log.d(Qapps.TAG, "Breadcrumb exceeds character limit: [" + recordLength + "], reducing it to: [" + maxBreadcrumbSize + "]");
            }
        ...
        if(logs.size() > maxBreadcrumbLimit){
            if(Qapps.sharedInstance().isLoggingEnabled()){
                Log.d(Qapps.TAG, "Breadcrumb amount limit exceeded, deleting the oldest one");
            }
            logs.removeFirst();
        }
    }
    ...
    static String getBatteryLevel(Context context) {
        try {
            ...
            if (Qapps.sharedInstance().isLoggingEnabled()) {
                Log.i(Qapps.TAG, "Can't get batter level");
            }
        }
    ...
    static String isOnline(Context context) {
        try {
            ...
            if (Qapps.sharedInstance().isLoggingEnabled()) {
                Log.w(Qapps.TAG, "Got exception determining connectivity", e);
            }
        }
        return null;
    }
     ...
     * http://resources.qassioun.com/v1.0/docs/i
     */
