package com.qassioun.android.sdk.messaging;
...
public class QappsPush {
    public static final String EXTRA_ACTION_INDEX = "com.qassioun.android.sdk.QappsPush.Action";
    public static final String EXTRA_MESSAGE = "com.qassioun.android.sdk.QappsPush.message";
    public static final String EXTRA_INTENT = "com.qassioun.android.sdk.QappsPush.intent";
    public static final String CHANNEL_ID = "com.qassioun.android.sdk.QappsPush.CHANNEL_ID";
    public static final String NOTIFICATION_BROADCAST = "com.qassioun.android.sdk.QappsPush.NOTIFICATION_BROADCAST";

    private static Application.ActivityLifecycleCallbacks callbacks = null;
    private static Activity activity = null;

    private static Qapps.QappsMessagingMode mode = null;

    @SuppressWarnings("FieldCanBeLocal")
    private static BroadcastReceiver notificationActionReceiver = null, consentReceiver = null;

    /**
     * Message object encapsulating data in {@code RemoteMessage} sent from Qapps server.
     */
    public interface Message extends Parcelable {
        /**
         * Qapps internal message ID
         *
    ...
    public static class NotificationBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent broadcast) {
            if(Qapps.sharedInstance().isLoggingEnabled()){
                Log.d(Qapps.TAG, "Push broadcast receiver receiving message");
            }

            broadcast.setExtrasClassLoader(QappsPush.class.getClassLoader());

            Intent intent = broadcast.getParcelableExtra(EXTRA_INTENT);
            intent.setExtrasClassLoader(QappsPush.class.getClassLoader());
...
//            }
    ...
    public static class ConsentBroadcastReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent broadcast) {
            if (mode != null && Qapps.sharedInstance().getConsent(Qapps.QappsFeatureNames.push)) {
    ...
    private static String getToken() {
        try {
            ...
            Log.e(Qapps.TAG, "Couldn't get token for Qapps FCM", logged);
            return null;
        }
    }
    ...
    /**
     * Standard Qapps logic for displaying a {@link Message}
     *
     ...
     * @return {@code Boolean.TRUE} if displayed successfully, {@code Boolean.FALSE} if cannot display now, {@code null} if no Qapps message is found in {@code data}
     */
    ...
    public static Boolean displayMessage(final Context context, final Message msg, final int notificationSmallIcon, final Intent notificationIntent) {
        if(Qapps.sharedInstance().isLoggingEnabled()){
            Log.d(Qapps.TAG, "Displaying push message");
        }
    ...
    /**
     * Standard Qapps logic for displaying a {@link Notification} based on the {@link Message}
     *
     * @param context context to run in
    ...
    public static Boolean displayNotification(final Context context, final Message msg, final int notificationSmallIcon, final Intent notificationIntent) {
        ...
        if(Qapps.sharedInstance().isLoggingEnabled()){
            Log.d(Qapps.TAG, "Displaying push notification");
        }
    ...
    /**
     * Standard Qapps logic for displaying a {@link AlertDialog} based on the {@link Message}
     *
    ...
    public static Boolean displayDialog(final Activity activity, final Message msg) {
        ...
        if(Qapps.sharedInstance().isLoggingEnabled()){
            Log.d(Qapps.TAG, "Displaying push dialog");
        }
        ...
        loadImage(activity, msg, new BitmapCallback() {
            ...
            public void call(Bitmap bitmap) {
                ...
                if (msg.media() != null) {
                    ...
                    throw new IllegalStateException("Qapps Message with UNKNOWN type in ProxyActivity");
                }
     ...
     *
     * @param token String token to be sent to Qapps server
     */
    public static void onTokenRefresh(String token) {
        ...
        if(Qapps.sharedInstance().isLoggingEnabled()){
            Log.d(Qapps.TAG, "Refreshing FCM push token");
        }
        Qapps.sharedInstance().onRegistrationId(token, mode);
    }
    ...
    public static void init(Application application, Qapps.QappsMessagingMode mode) throws IllegalStateException {
        if (!UtilsMessaging.reflectiveClassExists(FIREBASE_MESSAGING_CLASS)) {
            //throw new IllegalStateException("No FirebaseMessaging library in class path. Please either add it to your gradle config or don't use QappsPush.");
        }

        if(Qapps.sharedInstance().isLoggingEnabled()){
            Log.d(Qapps.TAG, "Initializing Qapps FCM push");
        }
        ...
        if (callbacks == null) {
            callbacks = new Application.ActivityLifecycleCallbacks() {
                ...
                public void onActivityStopped(Activity activity) {
                    if (activity.equals(QappsPush.activity)) {
                        QappsPush.activity = null;
                    }
                }
            ...
            filter.addAction(Qapps.CONSENT_BROADCAST);
